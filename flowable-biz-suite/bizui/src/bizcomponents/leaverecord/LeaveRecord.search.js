...
import styles from './LeaveRecord.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  const { LeaveRecordTypeAssociateForm } = GlobalComponents
  ...
  <div>
	...
	data={{leaveRecordList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'user')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'user')}/> <LeaveRecordTypeAssociateForm 
	visible={currentAssociateModal==='type'} 
	data={{leaveRecordList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'type')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'type')}/> <PlatformAssociateForm 
...
class LeaveRecordSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {LeaveRecordTable} = GlobalComponents;
    const {LeaveRecordSearchForm} = GlobalComponents;
    const {LeaveRecordModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <LeaveRecordSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,LeaveRecordModalTable,"leaveRecordIds")}
...
export default Form.create()(LeaveRecordSearch)
