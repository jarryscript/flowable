...
import styles from './LeaveRecord.table.less'
...
class LeaveRecordTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = LeaveRecordBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/leaveRecord/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
