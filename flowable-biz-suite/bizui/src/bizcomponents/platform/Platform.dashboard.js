...
class PlatformDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const { id,displayName, leaveRecordListMetaInfo, provinceListMetaInfo, cityListMetaInfo, districtListMetaInfo, leaveRecordCount, provinceCount, cityCount, districtCount } = this.props.platform
    ...
    const cardsData = {cardsName:"平台",cardsFor: "platform",
    	cardsSource: this.props.platform,returnURL,displayName,
  		subItems: [
{name: 'leaveRecordList', displayName: window.mtrans('leave_record','platform.leave_record_list',false) ,viewGroup:'__no_group', type:'leaveRecord',count:leaveRecordCount,addFunction: true, role: 'leaveRecord', metaInfo: leaveRecordListMetaInfo, renderItem: GlobalComponents.LeaveRecordBase.renderItemOfList},
