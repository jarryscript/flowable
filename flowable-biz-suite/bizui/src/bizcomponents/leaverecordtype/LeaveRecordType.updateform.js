...
import styles from './LeaveRecordType.updateform.less'
...
class LeaveRecordTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const leaveRecordTypeId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, leaveRecordTypeId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateLeaveRecordType`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextLeaveRecordTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'leaveRecordType',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}离开记录类型: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(LeaveRecordTypeUpdateForm))
