...
class UserSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
...
componentDidMount() {
  ...
  buildStringSearchParameters = (formValues, fieldName) => {
    ...
    return {
      userList: 1,
      'userList.searchField': fieldName,
      'userList.searchVerb': 'startsWith',
      'userList.searchValue': fieldValue,
    }
  }
  */
  ...
  handleSearch = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const {listName} = owner
		...
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'contains', 'avatar'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'contains', 'description'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'district'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'role'))
      ...
      dispatch({
        ...
        userSearchFormParameters: filterObjectKeys(fieldsValue),
 ...
 <Col md={8} sm={24}>
               ...
               {getFieldDecorator('role', {initialValue: tryinit('role')})(
               
               <SelectObject 
                 disabled={!availableForEdit('role')}
                 targetType={"role"} 
                 requestFunction={UserService.requestCandidateRole} useForSearch />
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label={fieldLabels.avatar}>
 ...
 <Col md={8} sm={24}>
                    <Form.Item label={fieldLabels.district}>
                  {getFieldDecorator('district', {initialValue: tryinit('district')})(
                  
                  <SelectObject 
                    disabled={!availableForEdit('district')}
                    targetType={"district"} 
                    requestFunction={UserService.requestCandidateDistrict} useForSearch />
