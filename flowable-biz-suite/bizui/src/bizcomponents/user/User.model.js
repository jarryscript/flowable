...
export default {
  ...
  effects: {
    ...
    *addLeaveRecord({ payload }, { call, put }) {
      ...
      const data = yield call(UserService.addLeaveRecord, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/user/${id}/list/LeaveRecordList/留下记录+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateLeaveRecord({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(UserService.updateLeaveRecord, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/user/${id}/list/LeaveRecordList/留下记录列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeLeaveRecordList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(UserService.removeLeaveRecordList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
