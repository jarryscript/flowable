...
import styles from './HolydaySetting.table.less'
...
class HolydaySettingTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = HolydaySettingBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/holydaySetting/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default HolydaySettingTable
