...
import com.doublechain.flowable.FlowableBaseConstants;
import com.doublechain.flowable.FlowableBaseUtils;
...
import com.doublechain.flowable.CustomFlowableCheckerManager;
import com.doublechain.flowable.restpageview.*;
...
/**
 * 此类负责：声明所有RestViewService中所使用的方法和常量。 单独列出的目的是便于维护。
 * @author clariones
 *
 */
public abstract class BaseRestViewService extends CustomFlowableCheckerManager implements FootprintProducer{
	...
	protected abstract void commonLog(CustomFlowableUserContextImpl ctx, String eventCode, String title, 
			String key1, String key2, String key3, Object detailInfo);
			
	protected boolean hasFormResubmitFlag(CustomFlowableUserContextImpl  ctx) {
		Object flag = ctx.getFromContextLocalStorage(FlowableBaseConstants.KEY_RE_SUBMIT);
		if (flag == null) {
			return false;
		}
	...
	public Object sendVerifyCode(CustomFlowableUserContextImpl ctx, String mobile) throws Exception {
		String vCode = RandomUtil.randomNum(6);
		mobile = FlowableBaseUtils.formatChinaMobile(mobile);
    ...
    protected String getSmsVCodeTemplate(CustomFlowableUserContextImpl ctx) {
		return TextUtil.getExtVariable("SMS_VERIFY_CODE", "");
	}
	protected String getSmsSign(CustomFlowableUserContextImpl ctx) {
		return TextUtil.getExtVariable("SMS_SIGN", "");
	}    
    protected User processClientLogin(CustomFlowableUserContextImpl ctx, LoginParam loginParam) throws Exception {
	...
	protected void afterLogined(CustomFlowableUserContextImpl ctx, User loginTarget) {
    	// 默认不用再做什么了
	}

	protected void saveTokenInResponse(CustomFlowableUserContextImpl ctx, String jwtToken) {
    	ctx.setResponseHeader(JWTUtil.HEADER_NAME, jwtToken);
	}

	protected String makeClientToken(CustomFlowableUserContextImpl ctx, String loginInfoCacheToken, User loginTarget) {
		String jwtTokenString = JWTUtil.getJwtToken(loginInfoCacheToken,
				FlowableBaseUtils.getOssUploadFolderName(User.INTERNAL_TYPE, loginTarget.getId(), ctx.isProductEnvironment()),
				ctx.isProductEnvironment()?"product":"test", "health",ctx.now());
		return jwtTokenString;
	}

	protected int getLoginInfoCachePeriod(CustomFlowableUserContextImpl ctx) {
		return (int) (7*DateTimeUtil.DAY_IN_MS/DateTimeUtil.SECOND_IN_MS);
	}
	protected void cacheLoginedUser(CustomFlowableUserContextImpl ctx, String loginInfoCacheToken, SecUser secUser, UserApp userApp, User loginTarget) {
	...
	protected String createLoginInfoCacheToken(CustomFlowableUserContextImpl ctx, User loginTarget) {
    	// 随机生成一个字符串
    	return FlowableBaseUtils.hashWithSHA256(loginTarget.getId(), RandomUtil.randomChars(8));
	}

	// 如果对应的secUser不存在，一般来说是代码或者数据异常，需要人工干预。 也可以不调查原因，直接创建secUser和userApp。 默认的实现是 抛异常。
    protected SecUser whenUserNotRelatedWithSecUser(CustomFlowableUserContextImpl ctx, BaseLoginHandler loginHandler, User loginTarget) throws Exception {
        throwsExceptionWithMessage(ctx, String.format("该用户(%s)没有关联系统账户，请联系客户处理账号异常。", loginTarget.getId()));
        return null;
    }

    protected void throwsExceptionWithMessage(CustomFlowableUserContextImpl ctx, String message) throws FlowableException {
    	throw new FlowableException(message);
    }

    protected SecUser findSecUserByLoginTarget(CustomFlowableUserContextImpl ctx, User loginTarget) {
    ...
    protected abstract User onNewLogin(CustomFlowableUserContextImpl ctx, LoginParam loginParam, BaseLoginHandler loginHandler) throws Exception;
    ...
    public void onAccess(BaseUserContext baseUserContext, String methodName, Object[] parameters) {
        ...
        CustomFlowableUserContextImpl ctx = (CustomFlowableUserContextImpl) baseUserContext;
        ctx.putToCache(FlowableBaseUtils.getCacheAccessKey(ctx), Boolean.TRUE, (int)(1*DateTimeUtil.HOUR_IN_MS/DateTimeUtil.SECOND_IN_MS)); // 1个小时
        ctx.putIntoContextLocalStorage(FlowableBaseConstants.CACHE_KEY_ACCESS_METHOD, methodName);
        ctx.putIntoContextLocalStorage(FlowableBaseConstants.CACHE_KEY_ACCESS_PARAMS, parameters);
    ...
    protected boolean shouldRefreshToken(CustomFlowableUserContextImpl ctx) {
	...
	protected boolean isMethodNeedLogin(CustomFlowableUserContextImpl ctx, String methodName, Object[] parameters) {
		// 默认用名字前缀来判断
		return methodName.startsWith("customer");
	}
	protected User tryToLoadUserFromCache(CustomFlowableUserContextImpl ctx) throws Exception {
    ...
    protected DecodedJWT getTokenFromClientRequest(CustomFlowableUserContextImpl ctx) {
		// 先从Header取
    ...
    protected String getJwtTokenFromCookie(CustomFlowableUserContextImpl ctx) {
		if (ctx.isProductEnvironment()) {
			return null;
		}
	...
	// 处理请求：查看首页
	public static String makeViewHomepageUrl(CustomFlowableUserContextImpl ctx){
		return makeUrl("viewHomepage");
	}
	// 处理请求：默认的客户端登录接口
	public static String makeClientLoginUrl(CustomFlowableUserContextImpl ctx, com.terapico.caf.baseelement.LoginParam loginParam){
		return makeUrl("clientLogin", loginParam);
	}
	// 处理请求：这个程序员很懒,什么也没留下
	public static String makeApplyForLeaveUrl(CustomFlowableUserContextImpl ctx){
	...
	protected abstract int processRequestViewHomepage(CustomFlowableUserContextImpl ctx) throws Exception;
	/** 处理请求：默认的客户端登录接口. 返回值：PRC_BY_DEFAULT: ;  */
	protected abstract int processRequestClientLogin(CustomFlowableUserContextImpl ctx) throws Exception;
	/** 处理请求：这个程序员很懒,什么也没留下. 返回值：PRC_BY_DEFAULT: ;  */
	protected abstract int processRequestCustomerApplyForLeave(CustomFlowableUserContextImpl ctx) throws Exception;
	/** 处理请求：这个程序员很懒,什么也没留下. 返回值：PRC_BY_DEFAULT: ;  */
	protected abstract int processRequestCustomerSubmitApplication(CustomFlowableUserContextImpl ctx) throws Exception;

	protected MePage assemblerMePage(CustomFlowableUserContextImpl ctx, String requestName)throws Exception {
		MePage page = new MePage();
		page.assemblerContent(ctx, requestName);
		return page;
	}
	protected ApplyForLeavePage assemblerApplyForLeavePage(CustomFlowableUserContextImpl ctx, String requestName)throws Exception {
		ApplyForLeavePage page = new ApplyForLeavePage();
		page.assemblerContent(ctx, requestName);
		return page;
	}
	protected HomePage assemblerHomePage(CustomFlowableUserContextImpl ctx, String requestName)throws Exception {
		HomePage page = new HomePage();
		page.assemblerContent(ctx, requestName);
		return page;
	}

	protected abstract void getCurrentUserInfo(CustomFlowableUserContextImpl ctx);
	protected abstract void ensureCurrentUserInfo(CustomFlowableUserContextImpl ctx) throws Exception;
}
