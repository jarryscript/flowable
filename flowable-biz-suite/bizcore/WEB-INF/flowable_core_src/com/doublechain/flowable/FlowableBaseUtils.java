...
public class FlowableBaseUtils {
	...
	public static String getCacheAccessKey(FlowableUserContext ctx) {
	...
	protected static BaseEntity loadCanCacheInLocal(FlowableUserContext userContext, String type, String id) throws Exception {
	...
	public static <T extends BaseEntity> List<T> collectReferencedObjectWithType(FlowableUserContext userContext,
	...
	private static <T extends BaseEntity> void collectReferedObjectIdSet(FlowableUserContext userContext,
	...
	public static int getAppBuildVersion(FlowableUserContext ctx) {
		return getBuildVersion(getRequestAppVersion(ctx));
	}
	protected static boolean startFromVersion(FlowableUserContext ctx, int version) {
     ...
     * "x-app-version" : "9"
	 */
	public static String getRequestAppVersion(FlowableUserContext userContext) {
		return userContext.getRequestHeader("x-app-version");
	}
	public static String getRequestAppDevice(FlowableUserContext userContext) {
		return userContext.getRequestHeader("x-app-device");
	}
	public static String getRequestAppType(FlowableUserContext userContext) {
	...
	public static Role getRole(FlowableUserContext userContext, String code) throws Exception {
		String key = "enum:" + Role.INTERNAL_TYPE + ":" + code;
		Role data = (Role) userContext.getFromContextLocalStorage(key);
		if (data != null) {
			return data;
		}
		data = userContext.getDAOGroup().getRoleDAO().loadByCode(code, emptyOptions);
		userContext.putIntoContextLocalStorage(key, data);
		return data;
	}


	public static LeaveRecordType getLeaveRecordType(FlowableUserContext userContext, String code) throws Exception {
		String key = "enum:" + LeaveRecordType.INTERNAL_TYPE + ":" + code;
		LeaveRecordType data = (LeaveRecordType) userContext.getFromContextLocalStorage(key);
		if (data != null) {
			return data;
		}
		data = userContext.getDAOGroup().getLeaveRecordTypeDAO().loadByCode(code, emptyOptions);
		userContext.putIntoContextLocalStorage(key, data);
		return data;
	}

	public static <T> T loadBaseEntityById(FlowableUserContext ctx, String type, String id) throws Exception {
		return loadBaseEntityById(ctx, type, id, null);
	}
	public static <T> T loadBaseEntityById(FlowableUserContext ctx, String type, String id, Consumer<T> enhancer) throws Exception {
		String key = type+":"+id;
	...
	public static <T extends BaseEntity> void appendLinkToUrl(FlowableUserContext ctx, List<T> list,
			Function<T, String> makeFunc) {
		if (list == null || list.isEmpty()) {
			return;
		}
		list.forEach(it -> {
			it.addItemToValueMap(FlowableBaseConstants.X_LINK_TO_URL, makeFunc.apply(it));
		});
	}

	public static <T extends BaseEntity> void appendLinkToUrl(FlowableUserContext ctx, T obj, String url) {
		if (obj == null) {
			return;
		}
		obj.addItemToValueMap(FlowableBaseConstants.X_LINK_TO_URL, url);
	}

	public static ButtonViewComponent addAction(FlowableUserContext ctx, BaseEntity obj, String title, String code,
			String linkToUrl) {
	...
	public static void setAction(FlowableUserContext ctx, BaseEntity obj, String title, String code, String linkToUrl) {
		ButtonViewComponent actionBtn = new ButtonViewComponent(title, null, code);
		actionBtn.setLinkToUrl(linkToUrl);
